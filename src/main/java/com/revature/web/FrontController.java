package com.revature.web;import java.io.IOException;
...
import com.revature.templates.MessageTemplate;public class FrontController extends HttpServlet {
	
	private static final long serialVersionUID = -4854248294011883310L;
	private static final UserController userController = new UserController();
	private static final ObjectMapper om = new ObjectMapper();	@Override
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		String[] portions = URI.split("/");//		System.out.println(Arrays.toString(portions));		try {
			try {
		switch(portions[0]) {
			case "users":
			...
			res.setStatus(401);
			MessageTemplate message = new MessageTemplate("The incoming token has expired");			res.getWriter().println(om.writeValueAsString(message));
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		res.setStatus(404);
		// Prevents non-desired endpoints from being successful
		final String URI = req.getRequestURI().replace("/ServletDemo", "").replaceFirst("/", "");		String[] portions = URI.split("/");		try {
			switch(portions[0]) {
			case "logout":
